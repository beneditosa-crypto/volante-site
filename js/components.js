...
export function imagemHtml(item, titulo) {
  const imagem = getImagem(item) || "https://placehold.co/900x700?text=Volante";
  const favorito = ehFavorito(item.id);

  return `
    <div class="foto-wrap">
      <button class="btn-favorito ${favorito ? "ativo" : ""}" onclick="event.stopPropagation(); toggleFavorito('${item.id}')" aria-label="Favoritar">
...
export function cardAnuncio(item) {
  const titulo = item.titulo || `${item.marca || ""} ${item.modelo || ""}`.trim() || "Anúncio";
  const local = textoLocal(item);
  const preco = formatarPreco(item.preco);

  return `
    <article class="card" onclick="window.location.href='./detalhe.html?id=${item.id}'">
      ${imagemHtml(item, titulo)}
      <div class="card-body">
        <div class="meta">${escapeHtml(local)}</div>
        ${preco ? `<div class="price">${escapeHtml(preco)}</div>` : ""}
      </div>
    </article>
  `;
}
...
export function cardEvento(item) {
  const titulo = item.titulo || item.nome || "Evento";
  const local = textoLocal(item);

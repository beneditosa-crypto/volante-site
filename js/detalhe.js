...
const tipoParam = String(params.get("tipo") || "").toLowerCase();
const colecao = tipoParam === "evento" ? "eventos" : "anuncios";
...
async function carregarDetalhe() {
  ...
  try {
    const itemRef = doc(db, colecao, id);
    ...
    if (item.status !== "ATIVO") {
      conteudo.innerHTML = `<div class="loading">Item indisponível.</div>`;
      return;
    }
...
function renderizar(item) {
  ...
  const ehEvento = colecao === "eventos";
  const tituloOriginal = item.titulo || item.nome || (ehEvento ? "Evento" : "Veículo anunciado");
  ...
  conteudo.innerHTML = `
    <section class="detalhe">
      ...
      <div class="painel">
        ...
        <div class="meta">${local || "Brasil"}</div>
        ...
        <div class="cta-app">
          ...
          <div class="app-store-box">
            <a class="app-store-btn" href="#" onclick="baixarApp(); return false;">
              <svg viewBox="0 0 24 24" fill="currentColor"><path d="M4 3.5v17l10.5-8.5L4 3.5Zm11.8 7.4 2.5-2.1L6.5 2.4l9.3 8.5Zm2.5 4.3-2.5-2.1-9.3 8.5 11.8-6.4Zm1.1-5.4-2.7 2.2 2.7 2.2 1.8-1c1-.6 1-2 0-2.6l-1.8-.8Z"/></svg>
              Google Play
            </a>

            <a class="app-store-btn" href="#" onclick="baixarApp(); return false;">
              <svg viewBox="0 0 24 24" fill="currentColor"><path d="M16.6 13.1c0-2.1 1.7-3.1 1.8-3.2-1-1.4-2.5-1.6-3-1.6-1.3-.1-2.5.8-3.1.8-.7 0-1.7-.8-2.8-.8-1.4 0-2.8.8-3.5 2.1-1.5 2.6-.4 6.4 1.1 8.5.7 1 1.5 2.2 2.7 2.1 1.1 0 1.5-.7 2.8-.7 1.3 0 1.7.7 2.8.7 1.2 0 1.9-1 2.6-2.1.8-1.2 1.1-2.3 1.2-2.4 0 0-2.6-1-2.6-3.4ZM14.5 6.9c.6-.7 1-1.7.9-2.7-.9 0-2 .6-2.6 1.3-.6.7-1.1 1.7-.9 2.7 1 .1 2-.5 2.6-1.3Z"/></svg>
              App Store
            </a>
          </div>
        </div>
        ...
        <div class="compartilhar">
          <div class="bloco">
            ...
            <div class="share-grid">
              ...
              <a class="share-btn share-facebook" href="${facebook}" target="_blank" rel="noopener" aria-label="Compartilhar no Facebook">
                <svg viewBox="0 0 24 24" fill="currentColor"><path d="M22 12a10 10 0 1 0-11.56 9.88v-6.99H7.9V12h2.54V9.8c0-2.5 1.49-3.89 3.77-3.89 1.09 0 2.23.2 2.23.2v2.46h-1.25c-1.24 0-1.63.77-1.63 1.56V12h2.77l-.44 2.89h-2.33v6.99A10 10 0 0 0 22 12"/></svg>
              </a>
            </div>
          </div>
        </div>
...
function iniciarGaleria() {
  ...
  fotoWrap.addEventListener("click", (event) => {
    ...
    posicaoX > largura / 2 ? proximaFoto() : fotoAnterior();
  });

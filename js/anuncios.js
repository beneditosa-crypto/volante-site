...
const gridAnuncios = document.getElementById("gridAnuncios");
...
function obterListaFiltrada() {
  return filtrarPorRegiao(
    filtrarPorBusca(anuncios)
  ).sort(ordenarAnuncios);
}
...
function atualizarTitulos() {
  ...
  if (regiaoAtual && NOMES_REGIOES[regiaoAtual]) {
    tituloPagina.textContent = `Anúncios no ${NOMES_REGIOES[regiaoAtual]}`;
    subtituloPagina.textContent =
      `Explore veículos clássicos e antigos da região ${NOMES_REGIOES[regiaoAtual]}.`;
    return;
  }

  tituloPagina.textContent = "Anúncios";
  subtituloPagina.textContent =
    "Explore veículos clássicos, antigos e especiais anunciados no Volante.";
}
  ...
  try {
    if (gridAnuncios) {
      gridAnuncios.innerHTML =
        `<div class="empty">Carregando anúncios...</div>`;
    }
    ...
    if (gridAnuncios) {
      gridAnuncios.innerHTML =
        `<div class="empty">Não foi possível carregar os anúncios agora.</div>`;
    }
